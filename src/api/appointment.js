...
export const createClientNote = async (appointmentId, data) => {
  const response = await axios.post(`/appointments/create-client-note/${appointmentId}`, data);
  return response.data;
};
export const getClientNotes = async (appointmentId) => {
  const response = await axios.get(`/appointments/client-notes/${appointmentId}`);
  return response.data;
};